	...
	if (graphId != null && graphId != '')
	{
		...
		xhr.onload = mxUtils.bind(this, function() {
			if (xhr.status >= 200 && xhr.status < 300)
			{
				...
				{
					this.setData(response.code);
					this.initialData = this.getData();
					this.ui.setFileData(response.code, this);
				}
			}
		});
		...
		else
		{
			...
			xhr.onload = mxUtils.bind(this, function() {
				...
				else
				{
					...
					if (error != null)
					{
						error({message: '保存失败: ' + xhr.statusText});
					}
				}
			});
			
			xhr.onerror = mxUtils.bind(this, function() {
				// 网络错误
				this.savingFile = false;
				
				if (error != null)
				{
					error({message: '网络错误，保存失败'});
				}
			});
 ...
 */
DrawioFile.prototype.getMode = function()
{
	return 'DRAWIO';
};
...
DrawioFile.prototype.saveDraft = function(data)
{
	try
	{
		...
		var draftData = (data != null) ? data : this.ui.getFileData();
		...
		xhr.onload = function() {
			...
			{
				// 保存成功
				EditorUi.debug('DrawioFile.saveDraft', 'API保存成功');
			}
			else
			{
				// 保存失败
				EditorUi.debug('DrawioFile.saveDraft', 'API保存失败: ' + xhr.statusText);
			}
		};
